...
### Q2：模型拉取失败/连接失败怎么办？
- 海外模型拉取失败：先检查 OpenRouter Key 与网络。
- 火山引擎连接失败：优先检查 `VOLCENGINE_BASE_URL` 是否为 `https://ark.cn-beijing.volces.com/api/v3`，再检查 `VOLCENGINE_API_KEY` 和模型名。
